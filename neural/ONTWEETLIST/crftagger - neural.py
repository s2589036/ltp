...
def main():
    ...
    for tweet in tweetlist:
        ...
        bigtaglist.append([tag for (word,tag) in tweet])
    print(bigtweetlist[0:10])
    print(bigtaglist[0:10])

    limit = round(len(bigtweetlist)*0.9)

    X_train = bigtweetlist[:limit] #the words
    y_train = bigtaglist[:limit] #the tags

    X_test = bigtweetlist[limit:] #the words
    y_test = bigtaglist[limit:] #the tags
    ...
    X_train_num = [[w2i[word] for word in tweet] for tweet in X_train]
    w2i = defaultdict(lambda: UNK, w2i) # freeze
    X_test_num = [[w2i[word] for word in tweet] for tweet in X_test]

    # same for labels/tags
    y_train_num = [[t2i[tag] for tag in tweet] for tweet in y_train]
    t2i = defaultdict(lambda: UNK, t2i) # freeze
    y_test_num = [[t2i[tag] for tag in tweet] for tweet in y_test]
    ...
    max_tweet_length = 140 #DIT NOG AANPASSEN

    X_train_pad = sequence.pad_sequences(X_train_num, maxlen=max_tweet_length, value=PAD)
    X_dev_pad = sequence.pad_sequences(X_test_num, maxlen=max_tweet_length, value=PAD)

    y_train_1hot_pad = sequence.pad_sequences(y_train_1hot, maxlen=max_tweet_length, value=TPAD)
    y_dev_1hot_pad = sequence.pad_sequences(y_test_1hot, maxlen=max_tweet_length, value=TPAD)
    ...
    vocabulary_size = 200*140 #DIT NOG AANPASSEN
    embeds_size = 100
     
    model = Sequential()
    model.add(Embedding(vocabulary_size, embeds_size, input_length=max_tweet_length,mask_zero=True))
